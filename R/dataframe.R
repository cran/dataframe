             ...
             stringsAsFactors = default.stringsAsFactors())
{
    ...
    vlistNames <- paste0("vlist", seq(along=vnames))
    nrows <- ncols <- integer(n)
    for(i in seq_len(n)) {
        ## do it this way until all as.data.frame methods have been updated
        x0 <- eval(parse(text = paste0("..", i))[[1]]) # copy of ..1 etc.
	xi <- if(is.character(x0) || is.list(x0))
                 as.data.frame(x0, optional = TRUE,
                               stringsAsFactors = stringsAsFactors)
        else as.data.frame(x0, optional = TRUE)
    ...
    value <- eval(parse(text = paste("c(",
                          ...
                          ")")))
    ## This drops i-th component if it has 0 columns
    ...
    function(x, i, j, drop = if(missing(i)) TRUE else length(cols) == 1)
{
    ...
    # This is missing the final condition: none missing & sorted,
    # because R does not have a fast way to check for any missing values.
...
`[[<-.data.frame` <- function(x, i, j, value)
{
	...
	if(!is.null(value)) {
        ...
        if(is.numeric(i) && i > nc){
          if(i != nc+1)
            stop("attempting to add a nonexistent column, would leave empty columns")
          newNames <- c(names(x), paste0("V", nc+1))
          if(newNames[nc+1] %in% names(x))
            newNames[nc+1] <- make.unique(newNames)[nc+1]
          i <- newNames[nc+1]
        }
...
`$<-.data.frame` <- function(x, name, value)
{
    ...
    # Tricky way to do x[[name]] <- value, avoiding methods
    x <- "[[<-"("oldClass<-"(x, NULL), name, value)
    class(x) <- cl
    return(x)
}
...
Math.data.frame <- function (x, ...)
{
	...
	x[] <- lapply(X = x, FUN = .Generic, ...)
        ...
        # skip non-numerical variables instead of failing
	vnames <- names(x)
	if (is.null(vnames)) vnames <- seq_along(x)
        warning("skipping non-numeric variable in data frame: ",
                vnames[!mode.ok])
        x[mode.ok] <- lapply(x[mode.ok], .Generic, ...)
    }
    return(x)
}
...
# Three lines from the chron package (in files chron.R dates.R times.R).
# chron has: License: GPL-2
as.data.frame.chron <- as.data.frame.vector
as.data.frame.dates <- as.data.frame.vector
as.data.frame.times <- as.data.frame.vector

# Lines from other files in R (dates.R datetime.R version.R)
